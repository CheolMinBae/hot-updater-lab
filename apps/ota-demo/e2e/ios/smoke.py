...
def run(*args, timeout=120, check=True, env=None):
    return subprocess.run(args, cwd=APP_ROOT, check=check, timeout=timeout,
...
def main():
    ...
    try:
        ...
        if completed.returncode:
            print((ARTIFACTS / "xcodebuild-test.log").read_text()[-18000:])
        ...
        for command in (
            ...
            try:
                ...
                if collected.returncode:
                    print(f"Evidence export: {collected.stderr.strip()}")
            except subprocess.SubprocessError as error:
                print(f"Evidence export failed: {error}")
        try:
            ...
            print(f"Simulator log export failed: {error}")
